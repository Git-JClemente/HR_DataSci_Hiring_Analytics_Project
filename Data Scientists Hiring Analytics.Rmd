...
("aug_train.csv") is really useful for the scope of this analysis as it
...
(aug_test.csv) is for verification that the developed supervised
learning model can accurately predict job interest, whereas the other
unused dataset file ("sample_submission.csv") is apparently sample
submission data (again, outside this project's scope.)

The 19,158 long/narrow formatted observations in our "aug_train.csv"
...
cleaning dataframe columns is the "clean_names" function. This function
...
cleaned output of the column names of our "DS_Candidate_Work_Data"
dataframe.
...
First, let's focus on the very first column, "enrollee_id". Based on
...
that row 5 has an enrollee_id value of "666" whereas the enrollee_id
...
the enrollee_id values to ensure they have exact the same number of
characters as the maximum \# found in this column.

Due to what identifiers typically represent, we shouldn't expect the
need to perform any mathematical operations on the "enrollee_id" values.
Therefore, it makes sense to convert these value types from "int" to
"chr".

Before we do this, we first check for the max \# of digits/characters in
the enrollee_id column, which shall serve as the standard number of
...
modify the enrollee_id column accordingly.
...
For "relevant_experience", since there are only 2 possible values, this
...
"gender" column to the "enrolled_university", "education_level",
"major_discipline", "company_type", "experience" and "last_new_job"
...
For the "company_size" column, there is an obvious typo for one of the
...
matching number of enrollee_ids as there are observations and that there
are no duplicate observatons.
...
First, let's examine the "city_development_index", which is directly
...
"city_development_index" to help characterize these cities. Per
...
top represented city (city_103) accounts for a significant 22.7%
(4355/19158) of all participants and boasts a very high CDI.
Interestingly, the second most represented city (city_21) which accounts
for 14.1% (2702/19158) has one of the lowest CDIs compared to the other
top 30 represented cities. This further reinforces the absence of any
correlation between CDI and City representation among the potential
candidates.
...
cities (city_11 & city_21) had ratios greater than 1. This indicates
...
candidates data (DS_Candidates_Work_Data_mod6), we'll create a new df
(called EducationData) to store all the columns that may be useful in
this section.
...
looking for a new job. Starting with our DS_Candidates_Work_Data_mod6
dataframe, we'll trim down to a more concise df that captures just the
columns of interest for this section.
...
threshold (\< 1 or \> 20). From personal experience, there isn't much of
a difference between someone with 7 years of experience vs. one with 8
years. Therefore, it may be more useful and convenient to group the
years of experience by intervals of 5 as follows. Additionally, we
...
the companys data science coursework. Other than the group with \<1 year
of relevant experience (where the count is exactly equal), the remaining
groups show that the count not looking for a career change in data
science (whether this be turnover to a new company as a current data
scientist or a career change) exceeds (sometimes drastically) the count
for those that are looking for a new job. This tells us that in order to
more successfully recruit experienced Data Scientists, the company
should look into career opportunities that already established data
science desire at later stages of their career. Perhaps this may mean
increased leadership opportunities or roles in mentoring younger data
scientists. There are obviously increased roles and responsibilities
that go along with the levels/years of experience. There are only a
limited number of these "higher" level roles in a company (not everyone
can be a lead data analyst/data manager). Having such a role in your
current company, more likely roots you into staying vs. early/mid career
data scientists with less than 10 years of experience, who can find
similar increased roles through job changes. Therefore, the company
should understand the general difficulty in recruiting more experienced
data scientists compared to earlier/mid career data scientists simply
due to the various structures and organizations that exist.
